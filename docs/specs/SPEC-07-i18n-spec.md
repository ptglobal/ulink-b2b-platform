...
- **Content:** Directus **Translations** on text-bearing fields of every content
  collection; frontend requests the active locale's translation.
...
## Fallback
Missing a content translation → fall back to VI (default) and flag for editors. Missing
a UI key → build/CI should catch; never render the raw key in production.

## Adding a locale / key
1. Add the locale to `routing.ts` + a `messages/<locale>.json`.
2. Enable the Directus translation for that language.
