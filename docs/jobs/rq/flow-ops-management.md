...
* **Phương án phối hợp (Hybrid):**
  ...
  * **Webhook + Custom Endpoint:** Đối với các luồng nghiệp vụ phức tạp (như gửi email mẫu HTML động, xóa/nạp cache Redis, revalidate Next.js, tích hợp ERP), Directus Flow chỉ đóng vai trò **Trigger (kích hoạt)** và gửi webhook thô. Toàn bộ logic xử lý chính sẽ được viết trong **Next.js Route Handlers (BFF)**.
...
* **Có**. Cấu hình Directus Flow để khi có lỗi xảy ra liên tục (hoặc webhook trả về mã lỗi 5xx từ Next.js/ERP):
  * Ghi chi tiết lỗi vào Directus Flow run history.
  * Trigger một webhook gửi cảnh báo khẩn cấp tới kênh Slack/Teams của đội ngũ kỹ thuật (hoặc email Admin).

### 6. Secrets / webhook token lưu ở đâu?
* **Bảo mật:** Lưu tập trung tại biến môi trường (`.env`) của Directus và Next.js.
* **Quy tắc:** Tuyệt đối không hardcode các chuỗi nhạy cảm (như mật khẩu Redis, Token Webhook `REVALIDATE_SECRET`, SMTP key) trong mã nguồn hoặc trong giao diện cấu hình Flow.
...
* **Directus Flows:** Đặt tên theo tiền tố `flow-` kèm chức năng: `flow-revalidate-content`, `flow-sku-cache-sync`, `flow-rfq-notification`.
* **Redis Key:** `sku:[code-lowercased]` (Ví dụ: `sku:cr-glv-001`).
* **Webhook Route:** `/api/revalidate`, `/api/sku/[code]`, `/erp/webhook`.
