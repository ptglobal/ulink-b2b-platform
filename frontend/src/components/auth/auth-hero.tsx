...
export async function AuthHero() {
  ...
  return (
    <aside className="relative hidden flex-1 flex-col overflow-hidden lg:flex">
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-between px-12 pb-7 pt-12">
        ...
        <div>
          ...
          <h1 className="mt-10 max-w-md whitespace-pre-line text-[2.5rem] font-bold leading-[1.15] tracking-tight text-primary">
            {t('heroTitle')} <span className="text-brand">{t('heroHighlight')}</span>
          </h1>
          <div className="mt-5 h-1 w-12 rounded bg-brand" />
          <p className="mt-5 max-w-sm text-sm leading-relaxed text-foreground/75">
            {t('heroDescription')}
          </p>
        </div>

        {/* Cụm dưới: badge + footer (KHÔNG nền sau text) */}
        <div>
          <ul className="flex flex-wrap gap-x-6 gap-y-4">
            {features.map(({ icon: Icon, title, desc }) => (
              <li key={title} className="flex max-w-[18ch] items-start gap-2.5">
                ...
                <span>
                  ...
                  <span className="block text-xs leading-snug text-foreground/70">{desc}</span>
                </span>
              </li>
            ))}
          </ul>
          <p className="mt-8 font-mono text-xs tracking-wider text-foreground/60">2026 ULINK INDUSTRIES.</p>
        </div>
      </div>
    </aside>
  );
