...
import {
  Search, Loader2, FileText, Clock, CheckCircle2, XCircle,
  Eye, ChevronRight, FileBox, X, Building2, User, Mail,
  Phone, MapPin, Factory, MessageSquare, Package, Truck, Calendar
} from 'lucide-react';
...
export function MyRfqsClient() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Detail Modal */}
      {selectedRfq && (
        <RfqDetailModal rfq={selectedRfq} onClose={() => setSelectedRfq(null)} />
      )}
    </div>
  );
}
...
function RfqDetailModal({ rfq, onClose }: { rfq: RfqItem; onClose: () => void }) {
  ...
  const ns = rfq.status === 'quoted' || rfq.status === 'won' ? 'quoted'
    : rfq.status === 'lost' || rfq.status === 'rejected' ? 'rejected'
    : 'pending';
          ...
          {(rfq.hub?.name || rfq.industry) && (
            <section>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {rfq.industry && <InfoRow icon={Factory} label={t('industry')} value={rfq.industry} />}
              </div>
            </section>
          )}
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left px-3 py-2 text-xs font-medium text-gray-500">{t('sku')}</th>
                      <th className="text-right px-3 py-2 text-xs font-medium text-gray-500 w-16">{t('qty')}</th>
                    </tr>
                  </thead>
          ...
          <section className="border-t border-gray-100 pt-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className={cn(
                ...
                ns === 'quoted' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' :
                ns === 'rejected' ? 'text-rose-700 bg-rose-50 border-rose-200' :
                'text-amber-700 bg-amber-50 border-amber-200'
              )}>
                {ns === 'quoted' && <CheckCircle2 className="h-3 w-3" />}
                {ns === 'rejected' && <XCircle className="h-3 w-3" />}
                {ns === 'pending' && <Clock className="h-3 w-3" />}
                {ns === 'quoted' ? t('//quoted') : ns === 'rejected' ? t('//rejected') : t('//pending')}
              </span>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs text-gray-400 mt-3">
              {rfq.source && (
                <span>{t('source')}: {rfq.source}</span>
              )}
              {rfq.date_created && (
                <span>{t('createdAt')}: {new Date(rfq.date_created).toLocaleDateString('vi-VN', {
...
function InfoRow({ icon: Icon, label, value }: { icon: typeof Building2; label: string; value?: string | null }) {
