...
export default async function CatalogShowcase({ locale, copy }: CatalogShowcaseProps) {
  const t = await getTranslations({ locale, namespace: 'solutions' });
  const label = (key: string) => copy?.[key] || t(`catalogSection.${key}`);
            ...
            return (
              <div key={catData.category.id} className="flex flex-col">
                ...
                <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-6">
                  ...
                  <Link
                    href={`/${locale}/products/categories/${catData.category.slug}`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {catData.products.map((product: Product) => {
                    ...
                    return (
                      <div
                        key={product.id}
                        className="group flex flex-col overflow-hidden border border-border bg-white transition-colors hover:border-brand"
                      >
                        {/* Product Image block */}
                        <Link
