...
import { Search, Loader2, Package, Eye, CheckCircle2, XCircle, User, Calendar, MapPin, Phone, Mail, Building, Clock, X, AlertTriangle, Plus, Trash2, Edit, Save, PlusCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { updateSampleRequestStatus, assignSampleRequestSales, saveSampleRequest, deleteSampleRequest } from '@/app/[locale]/admin/sample-requests/actions';
...
export function SampleRequestsClient({ initialRequests, salesTeam, skus, locale, error }: SampleRequestsClientProps) {
  ...
  const handleProcessSubmit = async (e: React.FormEvent) => {
    ...
    if (actionType === 'reject' && !rejectReason.trim()) {
      alert('Vui lòng nhập lý do từ chối yêu cầu hàng mẫu.');
      return;
    }
  ...
  const handleDeleteReq = async (id: number) => {
    if (!confirm('Bạn có chắc chắn muốn xóa yêu cầu hàng mẫu này? Thao tác này không thể hoàn tác.')) return;
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        alert('Không thể xóa yêu cầu hàng mẫu: ' + res.error);
      }
    });
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!activeReq.company || !activeReq.contact_name || !activeReq.email || !activeReq.phone || !activeReq.province || !activeReq.district || !activeReq.address_detail) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        alert('Không thể lưu yêu cầu hàng mẫu: ' + res.error);
      }
    });
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-6 mb-8">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1 leading-relaxed">
            Phê duyệt mẫu thử sản phẩm tĩnh điện, găng tay phòng sạch của doanh nghiệp và bàn giao nhân viên sales theo sát.
          </p>
        </div>
                    ...
                    <tr key={req.id} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-slate-400 font-medium text-[11px]">{req.phone}</span>
                        </div>
                      </td>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
                  Sản phẩm gốc yêu cầu mẫu: <span className="text-slate-500 font-bold">{selectedReq.product_slug}</span>
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-3 gap-y-2 text-xs">
                    ...
                    <span className="col-span-2 text-[#0F1E36] font-extrabold">{selectedReq.company}</span>

                    <span className="text-slate-400 font-bold">Người nhận:</span>
                    <span className="col-span-2 text-slate-700 font-semibold">{selectedReq.contact_name}</span>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-3 gap-y-2 text-xs">
                    ...
                    <span className="col-span-2 text-slate-600 font-semibold flex items-center gap-1">
                      ...
                      {selectedReq.created_at ? new Date(selectedReq.created_at).toLocaleString('vi-VN') : '---'}
                    </span>
                  </div>
                </div>
              </div>
                ...
                {selectedReq.skus && selectedReq.skus.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {selectedReq.skus.map((skuCode, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-150 text-xs font-mono font-extrabold text-[#0F1E36] select-all shadow-sm"
                      >
                        {skuCode}
                      </span>
                    ))}
              ...
              {selectedReq.message && (
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Tin nhắn / Yêu cầu thêm của khách hàng</span>
              ...
              <div className="border-t border-slate-100 pt-5 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {selectedReq.status === 'approved' ? 'Đã duyệt gửi mẫu' : selectedReq.status === 'rejected' ? 'Đã từ chối gửi' : 'Đang chờ xử lý'}
                    </span>
                  </div>
                  ...
                  <form onSubmit={handleProcessSubmit} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex flex-col gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="flex items-center justify-between border-b border-slate-150 pb-2 mb-1">
                      <h4 className="text-xs font-extrabold text-[#0F1E36] uppercase tracking-wider">
                        {actionType === 'approve' ? 'Phê duyệt gửi hàng mẫu' : 'Từ chối gửi mẫu thử'}
                      </h4>
                      <button
                        type="button"
                    ...
                    {actionType === 'approve' && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Gán nhân viên Sales chăm sóc *</label>
                          <select
                            required
                        ...
                        {/* Approval Note */}
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Ghi chú duyệt</label>
                          <input
                            type="text"
                            value={approvalNote}
                    ...
                    {actionType === 'reject' && (
                      <div className="flex flex-col gap-1.5">
                        <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Lý do từ chối gửi mẫu *</label>
                        <textarea
                          required
                          rows={2}
                          value={rejectReason}
                ...
                {selectedReq.status === 'approved' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-emerald-50/30 p-4 rounded-xl border border-emerald-100/50 text-xs animate-fade-in">
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-bold text-emerald-700 uppercase">Ghi chú gửi mẫu:</span>
                      <span className="text-emerald-900 font-semibold">{selectedReq.approval_note || 'Không có ghi chú.'}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-bold text-emerald-700 uppercase">Sales theo sát:</span>
                      <span className="text-emerald-900 font-extrabold flex items-center gap-1">
                        <User className="h-3.5 w-3.5 text-emerald-600" />
                        {selectedReq.assigned_sales
                          ? `${selectedReq.assigned_sales.first_name || ''} ${selectedReq.assigned_sales.last_name || ''}`.trim() || 'Salesperson'
                          : 'Chưa phân công'}
                      </span>
                    </div>
                  </div>
                ...
                {selectedReq.status === 'rejected' && (
                  <div className="bg-rose-50/30 p-4 rounded-xl border border-rose-100/50 text-xs flex flex-col gap-1 animate-fade-in">
                    ...
                    <span className="text-rose-900 font-semibold">{selectedReq.reject_reason || 'Không có lý do rõ ràng.'}</span>
                  </div>
                )}
              </div>
          ...
          <div className="my-8 w-full max-w-4xl bg-white rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h2 className="text-base sm:text-lg font-extrabold text-[#0F1E36] flex items-center gap-2">
                  <Package className="h-5 w-5 text-blue-500" />
                  {activeReq.id ? `Cập nhật yêu cầu hàng mẫu #${activeReq.id}` : 'Tạo yêu cầu hàng mẫu mới'}
                </h2>
                <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col max-h-[70vh] overflow-hidden">
              <div className="p-6 overflow-y-auto flex-1 bg-white grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  {/* Contact Name */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Người nhận mẫu *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Quận / Huyện *</label>
                      <input
                        type="text"
                        required
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Địa chỉ chi tiết *</label>
                    <input
                      type="text"
                      required
                      value={activeReq.address_detail || ''}
                      onChange={(e) => setActiveReq({ ...activeReq, address_detail: e.target.value })}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Đường dẫn sản phẩm gốc *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4 mt-1">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Trạng thái</label>
                      <select
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) => setActiveReq({ ...activeReq, assigned_sales_id: e.target.value || null })}
                        className="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 focus:outline-none bg-white shadow-sm"
                      >
                        <option value="">-- Chưa gán --</option>
                        {salesTeam.map((sales) => (
                  ...
                  {/* Message */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-extrabold text-slate-450 uppercase tracking-wider">Yêu cầu đính kèm</label>
                    <textarea
                      rows={3}
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-3 max-h-[45vh] overflow-y-auto pr-1">
                    {(activeReq.skus || []).map((skuCode: string, idx: number) => (
                      <div key={idx} className="p-4 border border-slate-150 rounded-xl bg-slate-50/30 flex items-center justify-between gap-3 relative animate-in fade-in duration-150">
                        <div className="flex-1 flex flex-col gap-1">
                          <label className="text-[9px] font-bold text-slate-450 uppercase">Chọn mã SKU mẫu *</label>
                          <select
                            required
                            value={skuCode}
                            onChange={(e) => handleUpdateFormItem(idx, e.target.value)}
                            className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-mono font-bold text-[#0F1E36] bg-white focus:outline-none"
                          >
